...
static void insertAfter(List* li, void* objet, Element* el){
    if(el==NULL) insertHead(List* li, objet);
...
void initListe(List* li){
    initListe(li, NONORDONNE, toString, compare);
}
...
List* createList(unsigned int type){
    createList(type, toString, compare);
}

List* createList(){
    createList(NONORDONNE,toString, compare);
}

boolean is_empty(Liste* li){
    return(li->nb==0);
}
...
void* extract_after(List* li, Element* previous){
    if(previous==NULL){
        extract_head(li);
    }else{
...
void insert_inOrder(List* li, void* object){
    if(is_empty(li)){
        ...
        if(inOrder(object,index->reference, li->type==1, li->compare)){
            ...
            insertAfter(li,previous,object);
        }        
    }
}
